<template>
    <section @touchstart="swipeStart" @touchmove="swipeMove" @touchend="swipeEnd" @touchcancel="swipeCancel">
        ...
        <div class="social">
            <div class="social__card" v-for="(post, index) in posts.data" :key="post.id" :style="`transform: translateX(${index}00%)`" :index="index">
                <IconsLightbox/>
    ...
    methods: {
        ...
        nextCard () {
            ...
            cards.forEach(card => {
                ...
                let test = card.getAttribute("index")
                ...
                test--;
                card.setAttribute("index", test)
                card.style.transform = `translateX(${ test }00%)`   
                // if (this.number < (this.posts.data.length - lastCards)) {
